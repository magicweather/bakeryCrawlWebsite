...
def fetch_hackney_relation():
    """Fetch the complete Hackney relation with geometry"""
    query = """
    [out:json][timeout:60];
    relation(51806);
    out geom;
...
def connect_boundary_ways(relation_data):
    ...
    relation = relation_data['elements'][0]  # The Hackney relation
    outer_ways = []
...
def main():
    print("🔧 Fixing Hackney boundary connection...")
    
    # Fetch data
    print("📡 Fetching relation data...")
    relation_data = fetch_hackney_relation()
    ...
    # Save fixed boundary
    with open('public/hackney-boundary.json', 'w') as f:
        json.dump(geojson, f, indent=2)
    
    print(f"✅ Fixed boundary saved with {len(connected_coords)} properly connected points!")
    print("🗺️ The boundary should now show smooth curves instead of straight lines.")
